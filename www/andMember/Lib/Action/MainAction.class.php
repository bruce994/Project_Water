	 ...
	 public function getAPI(){
	 	switch ($_GET['type']) {
	 		case 'getArchivesCount':
			 	$Model = new Model();
				$vo = $Model->query("SELECT count(*) as item1,FROM_UNIXTIME(stime,'%Y-%m-%d') as y  FROM `".C("DB_PREFIX")."shops_orders`  where mid=".$_SESSION [C ( 'USER_AUTH_KEY' )]." group by y order by y desc  limit 0,30");
