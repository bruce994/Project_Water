...
// 框架首页
	public function index() {
             ...
             if(isset($_GET['typeid'])){
                     $map['typeid'] = array('in',explode(',',$_GET['typeid'].getType2($_GET['typeid'])));
                }
...
$this->assign ( 'totalPages', $p->totalPages );
				...
				$this->assign ( 'backUrl', !empty($_REQUEST['backUrl'])?$_REQUEST['backUrl']:'');

                if($_GET['typeid'] == 193){  //积分商城
...
$this->assign ( 'totalPages', $p->totalPages );
		...
		public function cartDelete() {
			...
			$ids = explode ( ',', $id );
            if (isset ( $id )) {
                $condition = array ("id" => array ('in', $ids ),"mid"=>array("eq",$_SESSION["member"]['mid']) );
	...
	public function collect() {
		...
		public function receipt() {
			...
			$ids = explode ( ',', $id );
            if (isset ( $id )) {
                $condition = array ("oid" => array ('in', $ids ),"mid"=>array("eq",$_SESSION["member"]['mid']),"state"=>array("eq",1) );
				$list=$model->where ( $condition )->save(array("receipt"=>1));
                if ($list!==false) {
                    if(!empty($_GET['ajax'])){
                        echo '{"message":"操作成功！","code":"0","id":"'.$id.'"}';
                        exit;
                    }
					$this->success ('操作成功！' );
				} else {
                     if(!empty($_GET['ajax'])){
                        echo '{"message":"操作失败！","code":"1","id":"0"}';
                        exit;
                    }
					$this->error ('操作失败！');
				}
