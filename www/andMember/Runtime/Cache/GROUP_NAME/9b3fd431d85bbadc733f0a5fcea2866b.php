          ...
          <div class="row">
            <div class="col-xs-12">
              <div class="box">
                ...
                <form action="<?php echo U("Type/feedback",array("channel"=>$_GET['channel'],"typeid"=>$_GET['typeid'],"ss"=>4));?>" method="get">
                <div class="box-header">
                    
                    <div style="float:left;">
                      <a href="#" id="ids0" class="btn btn-block btn-default btn-sm" onclick="action('feedback_delete&id=','ids0');"> <i class="fa fa-fw fa-remove"></i>
                       删 除
                     </a>
                     </div>
                     ...
                     <div class="box-tools">
                        <div class="input-group" style="width: 150px;">
                          <input type="text" name="q" class="form-control input-sm pull-right" placeholder="<?php echo empty($_GET['q'])?"按评论内容搜索":$_GET['q'];?>">
                    ...
                    <tbody>
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td data-title="编号"><?php echo ($vo['id']); ?></td>
                             <td data-title="文章标题">
                               <?php  $tmp = M("Archives")->where("id=".$vo['aid'])->field("title")->find(); echo $tmp["title"]; ?>
                             </td>
                             <td data-title="评论时间">
                                <?php echo (date("Y-m-d H:i",$vo['dtime'])); ?></td>
                              <td data-title="评论人">
                                 <?php
 $member = M("Member")->where("mid=".$vo['mid'])->field("userid")->find(); echo $member['userid']; ?>
                              </td>                              
                              <td data-title="状态">
                                 <?php if(($vo["ischeck"]) == "1"): ?><span class="label label-success">已审</span><?php endif; ?>
                                 <?php if(($vo["ischeck"]) == "0"): ?><span class="label label-danger">待审</span><?php endif; ?>
                              </td>
                              <td data-title="操作">
                                <a class="btn btn-primary btn-xs" data-toggle="modal" data-target="#myModal<?php echo ($vo['id']); ?>">详细查看</a> | <a data-toggle="modal" data-target="#lc_confirm" onclick="lr_confirm('确定删除？','<?php echo U("Type/feedback_delete",array("id"=>$vo['id'],"channel"=>$vo['channel']));?>');"><i class="fa fa-fw fa-remove"></i></a>
                              </td>
                          </tr>
