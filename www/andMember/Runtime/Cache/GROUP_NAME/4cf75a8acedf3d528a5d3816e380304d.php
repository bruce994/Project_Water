          ...
          <div class="row">
            <div class="col-xs-12">
              <div class="box">
                ...
                <form id="form1" action="<?php echo U('Member/paySubmit');?>" method="post" class="pageForm required-validate">
                  ...
                  <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td data-title="订单号"><?php echo ($vo["buyid"]); ?></td>
                            <td data-title="产品名称"><?php echo ($vo["pname"]); ?>(<?php echo ($vo["oldinfo"]); ?>)</td>
                            <td data-title="生成日期"><?php echo (date("Y-m-d H:i:s",$vo["mtime"])); ?></td>
                            <td data-title="积分"><?php echo C('cfg_recharge')*$vo['money'];?></td>
                            <td data-title="金额"><?php echo ($vo["money"]); ?></td>
                            <td data-title="状态">
                                <?php if(($vo["sta"]) == "0"): ?><span class="label label-warning">未付款</span><?php endif; ?>
                                
                                <?php if(($vo["sta"]) == "1"): ?><span class="label label-success">已付款</span><?php endif; ?>

                            </td>
                            <td data-title="操作">
                                <?php if(($vo["sta"]) == "0"): ?><div class="col-md-8">
                                  <!-- USERS LIST -->
                                  <div class="box collapsed-box" style="margin-bottom:0px;border:none;background:none;box-shadow:none;">
                                    <div class="">
                                      <div class="">

                                       <a href="javascript:void(0);" class="btn btn-primary btn-xs" data-widget="collapse" ><i class="fa fa-plus"></i>去付款</a>
                                       <a href="javascript:void(0);"  class="btn btn-danger btn-xs"   data-toggle="modal" data-target="#lc_confirm" onclick="lr_confirm('确定删除？','<?php echo U("Member/deletePay",array("id"=>$vo['aid']));?>');" ><i class="fa fa-fw fa-remove"></i>删除</a>

                                      </div>
                                    </div><!-- /.box-header -->
                                    <div class="box-body">
                                     <?php  $payment = M("Payment")->select(); foreach ($payment as $value) { ?>
                                          <div style="margin:5px auto;">
                                                <a href="<?php echo U("Member/payUrl",array("aid"=>$vo['aid'],"payment"=>$value['code']));?>" target='_blank' class="btn btn-block btn-success btn-sm" onclick="lr_confirm('确定是否完成交易？','<?php echo U("Member/pay");?>');" > <i class="fa fa-fw  fa-credit-card"></i>
                                                 <?php echo $value['name'];?>
                                               </a>
                                          </div>
