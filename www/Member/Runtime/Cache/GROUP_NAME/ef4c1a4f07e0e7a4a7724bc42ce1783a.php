                ...
                <input type="hidden" name="mid" value="<?php echo $_SESSION [C ( 'USER_AUTH_KEY' )]; ?>"/>   
                  <div class="box-header">
                      <div style="text-align:center;">
                        <?php if($regis != '' ): ?><div class="alert alert-warning alert-dismissable">
                              <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                              <h4><i class="icon fa fa-warning"></i> 温馨提示!</h4>
                              您今天已经签到过了
                            </div>
                        <?php else: ?>
                            <button type="button" id="sub" class="btn btn-primary" type="button">点击这里签到赚积分</button><?php endif; ?>                        
                      </div>
                  </div><!-- /.box-header -->
                </form>
                  ...
                  <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                              <td><?php echo (date("m-d",$vo["addtime"])); ?></td>
                              <td>已签到</td>
