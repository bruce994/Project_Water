...
class UpyunAction extends CommonAction{
	...
	public function upload(){
		...
		if ($this->upload_type=='upyun'){
			...
			if (IS_POST){
				...
				echo '<script>location.href="?g=Member&m=Upyun&a=upload&error='.$return['error'].'&msg='.$return['msg'].'";</script>';
	...
	function localUpload($filetypes=''){
		...
		if (C("thumb")=="1"){
			if ((C("thumb_width"))&&intval(C("thumb_height"))){
				$upload->thumb = true;
				$upload->thumbMaxWidth=C("thumb_width");
				$upload->thumbMaxHeight=C("thumb_height");
				//$upload->thumbPrefix='';
				$thumb=1;
			}
	    }
        ...
        $upload->savePath = $_SERVER['DOCUMENT_ROOT'].'/upload/';// 设置附件上传目录
		...
		if(!$upload->upload()) {// 上传错误提示错误信息
			...
			if (C("thumb")=="1"){
				...
				$msg=C('site_url').'/upload/'.str_replace($fileName,'thumb_'.$fileName,$info[0]['savename']);
			}else {
				$msg=C('site_url').'/upload/'.$info[0]['savename'];
			}
		}
