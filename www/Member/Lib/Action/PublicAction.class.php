...
public function getOrderList(){
        ...
        if($_GET['valid'] == 'PJCydCf9'){
            $order = M("Shops_orders")->where(" state=0  and mid='".$_GET['uid']."'")->field("oid,mid,score")->select();
            $data = array();
            foreach($order as $v){
                $ml = C("cfg_milliliter") * $v['score'];
                $v['ml'] = $ml;
                $data[] = $v;
            }
       }else{
            $data = array("status"=>-1,"msg"=>"非法操作");
        }
...
public function qrcodeWater(){

        if($_GET['valid'] == 'PJCydCf9'){
            $paytype = empty($_GET['paytype'])?1:$_GET['paytype'];
            $order = M("Shops_orders")->where("oid='".$_GET['oid']."'")->find();
            $ml = C("cfg_milliliter") * $order['score'];
            $result = M("Shops_orders")->where("oid='".$_GET['oid']."'")->save(array("state"=>1,"paytype"=>$paytype,"takeTime"=>time()));
            if($result){
                $data = array("status"=>1,"score"=>$order['score'],"ml"=>$ml,"msg"=>"操作成功");

                //刷卡只扣卡积分，这里应恢复原订单积分
                if($paytype == 2){
                     //update user scores
                    $Model = new Model();
                    $Model->execute("update ".C("DB_PREFIX")."member set scores=scores+".$order['score']." where  mid=".$order['mid']);

                    $model = M ("Score_log" );
                    $model->add(array('mid'=>$_SESSION [C ( 'USER_AUTH_KEY' )],'typeid'=>-1,'aid'=>$aid,'addtime'=>time(),'score'=>"+".$score,'summary'=>'刷卡只扣卡积分,订单原积分恢复'));
                }
