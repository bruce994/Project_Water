	...
	public function article() {
				...
				if(!empty($_GET['channel'])){
					 $map['channel'] = array('eq',$_GET['channel']);
			 		 $channel = M ( "Channeltype" )->where("id=".$_GET['channel'])->find();
	                 ...
	                 foreach($channelField as $v){
	                        if($v['issearch'] == 1){
	                                if(!empty($_GET[$v['fieldname']])){
	                                        $map[$v['fieldname']] = array('like',"%".$_GET[$v['fieldname']]."%");
	                                }
	                        }
	                 }
	                 //./search
				...
				$typeid = $_SESSION["admin"]['typeid']; //操作栏目权限
				if($typeid){
...
public function feedback_check() {
	...
	if (isset ( $id )) {
		$condition = array ("id" => array ('in', $ids ) );
		$list=$model->where ( $condition )->save(array("ischeck"=>$this->_param("ischeck")));
		if ($list!==false) {
			$this->success ('操作成功！' );
		} else {
			$this->error ('操作失败！');
