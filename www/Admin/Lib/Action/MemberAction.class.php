...
// 框架首页
	public function index() {
				...
				if(!empty($_GET['userid'])){
					 $map['userid'] = array('like',"%".$_GET['userid']."%");
				}
				if(!empty($_GET['email'])){
					 $map['email'] = array('eq',$_GET['email']);
				}
                if(!empty($_GET['tel'])){
                    $person = M("Member_person")->where("tel like '%".$_GET['tel']."%'")->field("mid")->select();
                    $data = array();
...
$this->assign ( 'totalPages', $p->totalPages );
     ...
     public function pay(){
                                ...
                                if(!empty($_GET['buyid'])){
                                    $map['buyid'] = array('eq',$_GET['buyid']);
                                }
                                if($_GET['sta'] != 'aaa' && isset($_GET['sta'])){
                                    $map['sta'] = array('eq',$_GET['sta']);
                                }
...
public function deletePay() {
    ...
    $ids = explode ( ',', $id );
    if (isset ( $id )) {
        $condition = array ("aid" => array ('in', $ids ),"sta"=>array ('eq', 0 ) );
        $list=$model->where ( $condition )->delete();
        if ($list!==false) {
            $this->success ('删除成功！' );
        } else {
