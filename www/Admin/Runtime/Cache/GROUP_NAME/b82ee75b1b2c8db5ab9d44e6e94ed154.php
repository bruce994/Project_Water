            ...
            <div class="col-md-12">
              ...
              <div class="box box-primary">
                <div class="box-header with-border">
                  <h3 class="box-title"><?php echo ($vo["typename"]); ?></h3>
                </div><!-- /.box-header -->
                <!-- form start -->
                <form role="form" method="post" id="form1" action="<?php echo U("Type/update");?>" class="pageForm required-validate">
                  <input type="hidden" name="id" value="<?php echo ($vo["id"]); ?>" >
                  <div class="box-body">
                    <div class="form-group">
                      <label>频道模型</label>
                      <select name="channeltype" class="form-control">
                          <?php  $tmp = M("Channeltype")->field("id,typename")->select(); foreach($tmp as $v){ $select = ''; if($v['id'] == $vo['channeltype']){ $select = 'selected'; } echo '<option value="'.$v['id'].'" '.$select.'> '.$v['typename'].'</option>'; } ?>                      
                      </select>
                    </div>
                    <div class="form-group">
                      <label>栏目名称</label>
                      <input type="text" name="typename" class="form-control required"  value="<?php echo ($vo["typename"]); ?>">
                    </div>
                    <div class="form-group">
                      <label>排序</label>
                      <input type="text" name="sortrank"  class="form-control required number"  value="<?php echo ($vo["sortrank"]); ?>">
                    </div>

                    <div class="form-group">
                      <label>SEO标题</label>
                      <input type="text" name="seotitle" class="form-control"  value="<?php echo ($vo["seotitle"]); ?>">
                    </div>

                    <div class="form-group">
                      <label>关键字</label>
                      <input type="text" name="keywords" class="form-control"  value="<?php echo ($vo["keywords"]); ?>">
                    </div>

                    <div class="form-group">
                      <label>栏目描述</label>
                      <input type="text" name="description" class="form-control"  value="<?php echo ($vo["description"]); ?>">
                    </div>

                    <!-- 
                    <div class="form-group">
                      <label>封面模板</label>
                      <input type="text" name="tempindex" class="form-control required"  value="<?php echo ($vo["tempindex"]); ?>">
                    </div>
                     -->

                    <div class="form-group">
                      <label>栏目模板</label>
                      <input type="text" name="templist" class="form-control required"  value="<?php echo ($vo["templist"]); ?>">
                    </div>
                    <div class="form-group">
                      <label>文章模板</label>
                      <input type="text" name="temparticle" class="form-control required"  value="<?php echo ($vo["temparticle"]); ?>">
                    </div>                                        
               ...
               <div class="form-group">
                     ...
                     <br/>
                    <label>
                    <input class="minimal-red" type="radio" value = "0" name="ispart" <?php if(($vo["ispart"]) == "0"): ?>checked<?php endif; ?> > 最终列表栏目 
                    <small>（允许在本栏目发布文档，并生成文档列表）</small>
                    </label>
                    <br/>
                    <label>
                    <input class="minimal-red" type="radio" value = "1" name="ispart" <?php if(($vo["ispart"]) == "1"): ?>checked<?php endif; ?>> <small>频道封面（栏目本身不允许发布文档）</small>
                    </label>
                  </div> 

                  <div class="form-group" id="m_content" <?php if(($vo["ispart"]) == "0"): ?>style="display:none;"<?php endif; ?>  >
                      <label>栏目内容</label>
